    ...
    def send_message(self, message=None):
        try:
            ...
            if message:
                ...
                self.chat["DocId"] = int(self.selected_doc_id)
                self.chat["Question"] = message
                self.chat["History"] = self.history

                response = requests.post(self.BASE_URL + "docs/chat", json=self.chat)

                if response.status_code != 200:
                    raise Exception(
                        f'ChatApp:: chat error: {response.text}.\nStatus code: {response.status_code}')

                self.history.append({"Question": message, "Answer": response.json()})
                self.chat_display.config(state='normal')
    ...
    def upload_pdf(self, file_path):
        try:
            ...
            with open(file_path, 'rb') as f:
                ...
                response = requests.post(self.BASE_URL + 'docs/index', data=monitor, headers=headers)
    ...
    def on_file_click(self, event):
        try:
            ...
            if selected_index:
                ...
                self.selected_doc_id = text.split(": ")[1]
        except Exception as ex:
            messagebox.showerror('Error', 'Smth went wrong! Check logs for details')
